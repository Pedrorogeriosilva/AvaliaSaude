-- Dashboard e ranking com filtros de registros ativos
-- Execute este arquivo no SQL Editor do Supabase.
-- Script idempotente, seguro e não destrutivo.

create index if not exists idx_profiles_status on public.profiles(status);
create index if not exists idx_patients_status on public.patients(status);
create index if not exists idx_health_units_status on public.health_units(status);
...
where hu.status = 'active'
  and (
    ...
    or (
      p.id is not null
      and (e.created_by is null or pr.id is not null)
    )
  )
